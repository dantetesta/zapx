...
class EmailHelper {
    ...
    public static function send($to, $subject, $body, $isHtml = true) {
        ...
        try {
            ...
            // Validar se constantes estão PREENCHIDAS
            if (empty(SMTP_HOST) || empty(SMTP_USER) || empty(SMTP_PASS)) {
                error_log("❌ Configurações SMTP estão vazias. Configure o SMTP no config.php");
                return false;
            }
            
            if (empty(SMTP_FROM)) {
                error_log("❌ SMTP_FROM está vazio. Configure o email remetente");
                return false;
            }
            ...
            // Debug mode (descomente para ver detalhes)
            ...
            if ($encryption === 'ssl') {
                ...
                error_log("   Usando SSL (configurado manualmente)");
            } elseif ($encryption === 'tls') {
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // TLS/STARTTLS
                error_log("   Usando TLS/STARTTLS (configurado manualmente)");
            } else {
                // none - sem criptografia
                $mail->SMTPSecure = false;
                $mail->SMTPAutoTLS = false;
                error_log("   Sem criptografia (configurado manualmente)");
            }
            ...
            $mail->Timeout = 30; // 30 segundos para SSL
            $mail->SMTPOptions = [
                'ssl' => [
                    'verify_peer' => false,
