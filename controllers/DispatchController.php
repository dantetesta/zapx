...
class DispatchController extends Controller {
    ...
    public function send() {
        ...
        // 🔒 Marcar sessão como "em disparo" para timeout estendido (8 horas)
        $_SESSION['DISPATCHING'] = true;
        $_SESSION['LAST_ACTIVITY'] = time(); // Renovar timestamp
        ...
        // 🔒 SEGURANÇA: Não logar tokens completos
        error_log("Token: " . (empty($apiKey) ? 'não definido' : '[REDACTED]'));
        ...
        if ($mediaData && $mediaType === 'image') {
            require_once __DIR__ . '/../core/ThumbnailGenerator.php';
            $thumbGen = new ThumbnailGenerator();
            $thumbFilename = $thumbGen->generateFilename();
            
            $thumbnailPath = $thumbGen->generateImageThumbnail($mediaData['filepath'], $thumbFilename);
            
            if ($thumbnailPath) {
                error_log("✅ Thumbnail gerada: $thumbnailPath");
            }
        }
        ...
        $historyId = $this->dispatchModel->create($userId, $contactId, json_encode($historyData), $mediaType, $thumbnailPath);
        ...
        try {
            ...
            if ($result['success']) {
                ...
                // 🔒 Desmarcar flag de disparo (volta ao timeout normal de 2h)
                $_SESSION['DISPATCHING'] = false;
    ...
    private function processMediaUpload($file, $mediaType) {
        ...
        // 🔒 SEGURANÇA: Validar magic bytes (assinatura do arquivo)
        $magicBytesValid = $this->validateMagicBytes($file['tmp_name'], $mediaType);
        if (!$magicBytesValid) {
            error_log("❌ Magic bytes inválidos! Arquivo pode estar disfarçado.");
            return ['success' => false, 'message' => 'Arquivo inválido ou corrompido. O conteúdo não corresponde ao tipo declarado.'];
        }

        // Verificar tamanho baseado no tipo (limites configurados em config.php)
        $maxSizes = [
        ...
        $maxSize = $maxSizes[$mediaType] ?? UPLOAD_MAX_SIZE_IMAGE;
     ...
     */
    private function validateMagicBytes($filePath, $mediaType) {
        // Ler primeiros 12 bytes do arquivo
        $handle = fopen($filePath, 'rb');
        if (!$handle) {
            error_log("❌ Não foi possível abrir arquivo para validação");
            return false;
        }
        ...
        // Magic bytes conhecidos por tipo
        $magicBytes = [
            'image' => [
                'ffd8ff',           // JPEG
                '89504e47',         // PNG
                '47494638',         // GIF
                '52494646',         // WEBP (começa com RIFF)
            ],
            'video' => [
                '000000',           // MP4, MOV (pode começar com 00 00 00)
                '667479',           // MP4 (ftyp)
                '52494646',         // AVI (RIFF)
                '1a45dfa3',         // WEBM
            ],
            ...
            'document' => [
                '25504446',         // PDF
                'd0cf11e0',         // DOC, XLS (Microsoft Office)
                '504b0304',         // DOCX, XLSX (ZIP-based)
            ]
        ];
        
        if (!isset($magicBytes[$mediaType])) {
            error_log("⚠️ Tipo de mídia não tem validação de magic bytes: $mediaType");
            return true; // Permitir se não tiver validação específica
        }
        
        // Verificar se o início do arquivo corresponde a algum magic byte válido
        foreach ($magicBytes[$mediaType] as $magic) {
            if (str_starts_with($hex, $magic)) {
                error_log("✅ Magic bytes válidos: $magic");
                return true;
            }
        }
        
        error_log("❌ Magic bytes inválidos: $hex");
        error_log("   Esperado um de: " . implode(', ', $magicBytes[$mediaType]));
        return false;
    }
