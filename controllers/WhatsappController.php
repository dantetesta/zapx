...
class WhatsappController extends Controller {
    ...
    public function createInstance() {
        ...
        if ($apiSuccess) {
            ...
            // 🔒 SEGURANÇA: Não logar tokens completos
            error_log("Token extraído: " . (empty($instanceToken) ? 'NULL' : '[REDACTED]'));
            error_log("Resposta completa: [REDACTED - contém token]");
            error_log("Hash presente: " . (isset($apiResponse['hash']) ? 'SIM' : 'NÃO'));
            ...
            if ($instanceToken) {
                ...
                // 🔒 SEGURANÇA: Não logar tokens completos
                error_log("✅ Token será salvo: [REDACTED]");
            } else {
            ...
            // 🔒 SEGURANÇA: Não logar dados que contêm token
            error_log("Config Data: [REDACTED - contém token]");
            ...
            // 🔒 SEGURANÇA: Não logar tokens completos
            error_log("Dados após update - evolution_instance_token: " . (empty($userAfter['evolution_instance_token']) ? 'NULL' : '[REDACTED]'));
