...
// Criar objeto Logger completo para compatibilidade
window.Logger = {
    log: function(message, type) { window.log(message, type); },
    error: function(message) { window.log(message, 'error'); },
    success: function(message) { window.log(message, 'success'); },
    warning: function(message) { window.log(message, 'warning'); },
    info: function(message) { window.log(message, 'info'); }
};

// Atalhos para facilitar o uso
window.logInfo = function(message, data) { 
    if (data) console.log(message, data);
    else window.log(message, 'info'); 
};
window.logError = function(message, error) { 
    if (error) console.error(message, error);
    else window.log(message, 'error'); 
};
window.logSuccess = function(message) { window.log(message, 'success'); };
window.logWarning = function(message) { window.log(message, 'warning'); };

// ⚡ VERSÃO: <?php echo SYSTEM_VERSION; ?> - Build: <?php echo time(); ?>
console.log('🔄 Script carregado:', new Date().toISOString());
console.log('📦 Build:', <?php echo time(); ?>);
console.log('📌 Versão:', '<?php echo SYSTEM_VERSION; ?>');
log('✅ Logger carregado com sucesso!');

// Variáveis globais
let statusInterval;

document.addEventListener('DOMContentLoaded', function() {
    log('🚀 WhatsApp Manager carregado');
    log('✅ DOM carregado');
    log('✅ Cache limpo - Sistema funcionando corretamente!');
    ...
    if (autoOpenFromURL || autoOpenFromPHP) {
        log('✅ Vai abrir QR Code automaticamente em 800ms...');
        // Aguardar um pouco para garantir que a página carregou
        setTimeout(() => {
            log('🎯 Tentando abrir QR Code agora...');
            // Tentar ambas as funções
            if (typeof mostrarQRCode === 'function') {
                log('✅ Chamando mostrarQRCode()');
                mostrarQRCode();
            } else if (typeof gerarQRCode === 'function') {
                log('✅ Chamando gerarQRCode()');
                gerarQRCode();
            } else {
                logError('❌ Nenhuma função de QR Code encontrada!');
            }
        }, 800);
    } else {
        log('⏭️ Não vai abrir QR Code automaticamente');
    }
});
        ...
        if (data.success) {
            ...
            if (data.connected) {
                ...
                if (btnConectar) {
                    ...
                    log('🔒 Botão QR Code ocultado - WhatsApp conectado');
                ...
                if (wasConnecting) {
                    logSuccess('🎉 WhatsApp conectado! Recarregando página...');
