                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-green-500 transition-colors">
                                ...
                                <div id="filePreview" class="hidden">
                                    ...
                                    <div id="imagePreview" class="hidden relative">
                                        <img id="imagePreviewImg" src="" alt="Preview" class="w-full object-cover rounded-lg" style="height: 300px;">
...
<!-- Botão Flutuante para Reabrir Fila -->
<button id="reopenQueueBtn" 
        onclick="openQueueOffcanvas()" 
        class="fixed bottom-6 right-6 w-16 h-16 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-full shadow-2xl hover:shadow-purple-500/50 transition-all duration-300 flex items-center justify-center z-30 hidden animate-bounce"
        title="Abrir Fila de Disparo">
    <div class="relative">
        <i class="fas fa-list-ul text-xl"></i>
        <span id="queueBadge" class="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">0</span>
    </div>
</button>

<script>
// ⚡ Sistema de log simplificado (sem dependências externas)
window.log = function() { console.log.apply(console, arguments); };
window.logInfo = function() { console.info.apply(console, arguments); };
window.logWarn = function() { console.warn.apply(console, arguments); };
window.logError = function() { console.error.apply(console, arguments); };
window.logSuccess = function() { console.log.apply(console, arguments); };

// Compatibilidade com Logger (se existir)
if (typeof window.Logger === 'undefined') {
    window.Logger = {
        log: window.log,
        info: window.logInfo,
        warn: window.logWarn,
        error: window.logError,
        success: window.logSuccess
    };
}
...
let heartbeatInterval = null; // Intervalo para manter disparo ativo em background
...
function updateMessagePreview() {
    ...
    // Atualizar preview (sanitizado contra XSS)
    if (messageText.trim()) {
        ...
        // Substituir {nome} por exemplo (sanitizado)
...
// Atualizar preview de mídia no WhatsApp (protegido contra XSS)
function updateMediaPreviewWhatsApp() {
    ...
    if (selectedFile) {
        ...
        if (currentMediaType === 'image') {
            // Preview realista de imagem (DOM seguro)
            const container = document.createElement('div');
            container.className = 'relative rounded-md overflow-hidden mb-2';
            ...
            overlay.className = 'absolute bottom-0 right-0 bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded-tl-md';
            overlay.innerHTML = `<i class="fas fa-image mr-1"></i>${safeFileName}`;
            ...
            // Preview realista de vídeo (DOM seguro)
            const container = document.createElement('div');
            container.className = 'relative rounded-md overflow-hidden mb-2';
            ...
            const overlay = document.createElement('div');
            overlay.className = 'absolute top-2 left-2 bg-black bg-opacity-70 text-white text-xs px-2 py-1 rounded';
            overlay.innerHTML = '<i class="fas fa-video mr-1"></i>Vídeo';
            ...
            // Preview realista de áudio (DOM seguro)
            const container = document.createElement('div');
            container.className = 'bg-white rounded-md p-3 mb-2 flex items-center gap-3';
            
            const icon = document.createElement('div');
            icon.className = 'w-10 h-10 bg-green-500 rounded-full flex items-center justify-center';
            icon.innerHTML = '<i class="fas fa-microphone text-white"></i>';
            
            const audioContainer = document.createElement('div');
            audioContainer.className = 'flex-1';
            ...
            // Preview realista de documento (DOM seguro)
            ...
            const icon = document.createElement('div');
            icon.className = 'w-12 h-12 bg-blue-100 rounded flex items-center justify-center';
            icon.innerHTML = '<i class="fas fa-file-alt text-blue-600 text-xl"></i>';
            ...
            details.textContent = `${safeExtension} • ${formatFileSize(selectedFile.size)}`;
...
function openQueueOffcanvas() {
    ...
    const reopenBtn = document.getElementById('reopenQueueBtn');
    
    offcanvas.style.display = 'block';
    backdrop.style.display = 'block';
    
    // Esconder botão flutuante quando offcanvas está aberto
    if (reopenBtn) {
...
function closeQueueOffcanvas() {
    ...
    setTimeout(() => {
        ...
        // Mostrar botão flutuante se ainda estiver em disparo
        if (isDispatching && !isStopped && reopenBtn) {
...
async function startDispatch() {
    ...
    // Iniciar heartbeat para manter disparo ativo em background
    startHeartbeat();
    ...
    // Avisar usuário sobre background
    showNotification('💡 Mantenha esta aba aberta para melhor performance', 'info');
    console.log('💡 DICA: O sistema tentará continuar em background');
    console.log('   Mas alguns navegadores podem pausar quando a aba não está visível');
}
...
function startHeartbeat() {
    ...
    // Criar intervalo que verifica a cada 500ms
    heartbeatInterval = setInterval(() => {
        // Se não está em disparo, parar heartbeat
        if (!isDispatching || isStopped) {
            stopHeartbeat();
            return;
        }
        ...
        // Verificar se já passou tempo suficiente desde o último processamento
        const now = Date.now();
        if (now - lastProcessTime >= 500) {
            ...
            // Log para debug
            console.log('💓 Heartbeat ativo - Mantendo disparo em background');
        }
    }, 500);
    
    console.log('✅ Heartbeat iniciado - Disparo funcionará mesmo em background');
}
...
function renderQueue() {
    ...
    dispatchQueue.forEach((item, index) => {
        ...
        // Tratar nome vazio/null
        const displayName = item.name && item.name.trim() ? item.name : 'Sem nome';
...
async function processQueue() {
    ...
    try {
        ...
        formData.append('contact_name', item.name || ''); // Tratar nome vazio
        formData.append('contact_phone', item.phone);
        formData.append('message', item.message);
        formData.append('media_type', item.hasMedia ? currentMediaType : 'text');
        
        // Adicionar arquivo se houver
        if (selectedFile && item.hasMedia) {
            formData.append('media_file', selectedFile);
            console.log('📎 Arquivo anexado:', selectedFile.name, selectedFile.size, 'bytes');
        }
...
function finishDispatch() {
    ...
    // Desativar processamento em background
...
function updateMessageType() {
    console.log('🔍 updateMessageType() chamada');
...
function handleFileSelect(event) {
    ...
    console.log('📸 URL criada:', fileURL);
    ...
    if (mediaType === 'image') {
        ...
        console.log('🖼️ Elemento de imagem:', imgElement);
        
        if (imgElement) {
            imgElement.src = fileURL;
            console.log('✅ SRC definido:', imgElement.src);
            
            // Adicionar evento de erro para debug
            imgElement.onerror = function() {
                console.error('❌ Erro ao carregar imagem!');
                console.error('URL:', fileURL);
                console.error('Tipo do arquivo:', file.type);
            };
            ...
            console.error('❌ Elemento imagePreviewImg não encontrado!');
        }
...
// ========================================
// SISTEMA ANTI-THROTTLE PARA BACKGROUND
// ========================================
...
// Page Visibility API - Detectar quando aba fica inativa
document.addEventListener('visibilitychange', function() {
    isTabVisible = !document.hidden;
    
    if (document.hidden) {
        console.log('⚠️ Aba ficou inativa - Ativando modo background');
        enableBackgroundProcessing();
    } else {
        console.log('✅ Aba voltou a ficar ativa - Modo normal');
        disableBackgroundProcessing();
    }
});

// Ativar processamento em background
function enableBackgroundProcessing() {
    if (!isDispatching) return;
    
    console.log('🔄 Iniciando processamento em background...');
    
    // Usar setInterval ao invés de setTimeout para garantir execução
    if (!backgroundCheckInterval) {
        backgroundCheckInterval = setInterval(() => {
            const now = Date.now();
            const timeSinceLastProcess = now - lastProcessTime;
            
            // Se passou mais de 2 segundos sem processar, forçar processamento
            if (timeSinceLastProcess > 2000 && isDispatching && !isPaused && !isStopped) {
                console.log('⚡ Forçando processamento em background');
                lastProcessTime = now;
                // Não chamar processQueue aqui, apenas garantir que está rodando
            }
        }, 1000); // Verificar a cada 1 segundo
    }
    
    // Tentar manter Wake Lock
    requestWakeLock();
}

// Desativar processamento em background
function disableBackgroundProcessing() {
    if (backgroundCheckInterval) {
...
// Wake Lock API - Tentar manter tela ativa (se suportado)
let wakeLock = null;
async function requestWakeLock() {
    try {
        if ('wakeLock' in navigator) {
            wakeLock = await navigator.wakeLock.request('screen');
            console.log('🔒 Wake Lock ativado - Tela não vai desligar');
            
            wakeLock.addEventListener('release', () => {
                console.log('🔓 Wake Lock liberado');
                // Tentar reativar se ainda estiver disparando
                if (isDispatching && !isTabVisible) {
                    setTimeout(() => requestWakeLock(), 1000);
                }
            });
        } else {
            console.log('⚠️ Wake Lock não suportado neste navegador');
        }
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Tentar ativar Wake Lock quando iniciar disparo
    // (será ativado quando o usuário clicar em "Iniciar Disparo")
});
</script>
